...
void Sort(LinkNode*& L) {
	LinkNode* p, * q, * pre;
		p = L->next->next;      //第一个数据节点作为已排序好的序列
		L->next->next = NULL;
	while (p != NULL)       //插入排序
	{
		q = p->next;
		pre = L;       //每一次pre从头开始遍历已断了的L链表（已排序好的表）
		while (pre->next != NULL && pre->next->data > p->data) //开始遍历查找正确的插入位置(插入处为)
			pre = pre->next;    //未遍历到已排序链表的末端 并且 待插入数(p->data)比已排序链表当前数值(pre->next->data)小的，pre往后走
		p->next = pre->next; //已找到正确位置，将p节点插入到pre节点的后面(由此语句与下条语句两条语句完成)注释太多，word一行放不下所以看起来不整齐
		pre->next = p;
		p = q;    //p沿未排序链表往前走，指向下一个待插入元素
	}
	LinkNode* a=L->next;       //输出链表,并删除
	LinkNode* b=L;
